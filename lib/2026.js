...
function compute(group, template, deliverables, charter) {
    ...
    const workMode = (() => {
        ...
        if (oldDeliverables) {
            const draftState = oldDeliverables.querySelectorAll('p').item(1).textContent;
            if (draftState.includes('projected to become a Recommendation')) {
                return 'rec';
            } else if (draftState.includes('not intend to advance their documents to Recommendation')) {
                return 'cr-snapshot';
            }
        ...
        if (oldSuccess) {
            const draftState = oldSuccess.querySelectorAll('p').item(0).textContent;
            if (draftState.includes('advance beyond Candidate Recommendation')) {
                return 'rec';
            } else if (draftState.includes('advance beyond Proposed Recommendation')) {
                return 'rec';
            } else {
                return 'cr-snapshot';
            }

...
function loadSources() {
    return Promise.all([
        ...
        html(sources('charter'), { credentials: 'include', signal: abortSignal() }),
...
function generate() {
    Promise.all([
        loadSources().then(([group, template, deliverables, charter, templateCommits]) => compute(group, template, deliverables, charter, templateCommits)),
...
async function compute(group, template, deliverables, charter, templateCommits) {
...
async function fetchGroupData() {
    ...
    await Promise.all(["charters" ].map(async (i) => await extend(group, i)));
    group["latest-charter"] = group.charters.reduce((a, b) => {
        return new Date(a.end) > new Date(b.end) ? a : b;
    })

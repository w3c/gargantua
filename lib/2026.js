...
function compute(group, template, deliverables, charter) {
    ...
    if (templateDeliverables && group['group-type'] === 'ig') {
        ...
        if (normative) {
            ...
            parentNode.insertBefore(document.createComment(`using ${sources('deliverables')}`), dl);
    ...
    if (head) {
        ...
        scriptConfig.type = 'application/ld+json';
        const savedConfig = {};
        savedConfig.generator = 'charter-assistant';
        savedConfig.workMode=`${config.workMode}`;
        savedConfig.workModeFromDeliverables=`${config.workModeFromDeliverables}`;
        savedConfig.workModeFromSuccessCriteria=`${config.workModeFromSuccessCriteria}`;
        savedConfig.group = config.group;
        savedConfig.timeStamp = new Date().toISOString();
        scriptConfig.textContent = JSON.stringify(savedConfig, null, 2);

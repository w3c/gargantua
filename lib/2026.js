...
const delay = (seconds = 1) => new Promise(resolve => setTimeout(resolve, seconds*1000));

// 10s max per fetch request
function abortSignal(seconds = 10) { 
...
function loadSources() {
    return Promise.all([
        fetchGroupData(),
        html(sources('template'), { signal: abortSignal() }),
        html(sources('deliverables'), { credentials: 'include', signal: abortSignal() }).then(doc => {
            return doc.getElementById('deliverables').parentNode.querySelectorAll('dl');
        }),
        json(sources('template-commits'), { signal: abortSignal() }),
    ]);
}

function generate() {
    Promise.all([
        loadSources().then(([group, template, deliverables, templateCommits]) => compute(group, template, deliverables, templateCommits)),
        delay(5) // ensure the status is shown
    ]).then(([draft, timeOut]) => {
    ...
    if (checkGroups.length > 0) {
        ...
        checkGroups.forEach(a => {
            ...
            if (!match) {
                ...
                match = groups.find(g => g.homepage === href || g.url?.startsWith(href));
            }
...
async function fetchGroupData() {
    const orig_group = await w3cgroup(config.group).catch(err => err);
    ...
    async function extend(targetObject, rel, postFunction) {
        const entry = targetObject._links[rel];
        const url = entry.href || null;
        if (url) {
            targetObject[rel] = [];
            try {
                const items = await hal(url, rel, { embed: true, adapter: postFunction, fetch: { signal: abortSignal() } } );
                for await (const service of items) {
                    targetObject[rel].push(service);
                }
            } catch (err) {
                failed(`Could not load ${rel} data from W3C API: ${err.message}`);
                throw new Error('abort', { cause: err });
            }
            return targetObject[rel];
        }
    }
    ...
    group["active-charter"] = await json(group._links["active-charter"].href, { signal: abortSignal() });
    status(`Loading charter content ${group['active-charter'].uri} ...`);
    group.charterDoc = await html(group["active-charter"].uri, { credentials: 'include', signal: abortSignal() });
    
    if (config.debug) console.log(group);
    return group;
}

const w3cgroups = json(sources('groups'), { signal: abortSignal() }).then(all => {
    all.forEach(g => {
        g.homepage = g._links?.homepage?.href || null;
        g.url = g._links.self.href.replace(/^https:\/\/api.w3.org\/groups\//, `https://www.w3.org/groups/`) + '/';
    })
    return all;
});

const w3cgroup = async (identifier) => {
    const groups = await w3cgroups;

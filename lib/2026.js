...
function loadSources(identifier) {
    return Promise.all([
        ...
        html(sources('deliverables', identifier), { credentials: 'include', signal: abortSignal() }).then(doc => {
...
async function compute(group, template, deliverables, templateCommits) {
    ...
    if (templateDeliverables && group['group-type'] === 'ig') {
        ...
        if (normative) {
            ...
            parentNode.insertBefore(document.createComment(`  ${sources('deliverables', identifier)} `), dl);
...
async function w3cgroups() {
    ...
    return json(sources('groups'), { signal: abortSignal() }).then(all => {
        all.forEach(g => {

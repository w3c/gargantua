...
function sources(identifier, type) {
    status(`Loading ${type}`);
    switch (type) {
        case 'groups':
...
function loadSources(identifier) {
    return Promise.all([
        ...
        html(sources(identifier, 'template'), { signal: abortSignal() }),
        html(sources(identifier, 'deliverables'), { credentials: 'include', signal: abortSignal() }).then(doc => {
            return doc.getElementById('deliverables').parentNode.querySelectorAll('dl');
        }),
        json(sources(identifier, 'template-commits'), { signal: abortSignal() }),
    ]);
}
...
function generate(identifier) {
    Promise.all([
        ...
        addExportButton(`charter-${identifier}-${new Date().getFullYear()}.html`);
    }).catch(err => {
...
async function compute(group, template, deliverables, templateCommits) {
    ...
    if (templateDeliverables && group['group-type'] === 'ig') {
        ...
        if (normative) {
            ...
            parentNode.insertBefore(document.createComment(`  ${sources(identifier, 'deliverables')} `), dl);
...
async function w3cgroups() {
    ...
    return json(sources(identifier, 'groups'), { signal: abortSignal() }).then(all => {
        all.forEach(g => {
